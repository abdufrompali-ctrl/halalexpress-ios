...
struct CartLine: Identifiable, Equatable {
    ...
    var unitCents: Int { Int((item.price * 100).rounded()) }
...
final class CartStore: ObservableObject {
    ...
    func add(_ item: CatalogItem, option: String?, customizations: String?, quantity: Int) {
        ...
        if let idx = lines.firstIndex(where: {
            ...
            lines.append(CartLine(item: item, option: option,
                                  customizations: customizations, quantity: quantity))
        }
    }
